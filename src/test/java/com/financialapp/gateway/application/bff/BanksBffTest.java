...
class BanksBffTest {
    ...
    @BeforeEach
    void setUp() {
        useCase = new GetBanksBffUseCaseImpl(banks, upload, PageTimeoutBudget.fromMillis(3000));
    }

    @Test
    void execute_returnsOkSectionsWhenAllDownstreamsSucceed() {
        when(banks.fetchAccounts(any())).thenReturn(CompletableFuture.completedFuture(List.of(Map.of("id", 1))));
        when(banks.fetchCards(any())).thenReturn(CompletableFuture.completedFuture(List.of()));
        when(banks.fetchActiveLoans(any())).thenReturn(CompletableFuture.completedFuture(List.of()));
